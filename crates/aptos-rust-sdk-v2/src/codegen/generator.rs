...
use crate::codegen::types::{to_pascal_case, to_snake_case, MoveTypeMapper};
...
impl<'a> ModuleGenerator<'a> {
    ...
    fn write_header(&self, output: &mut String) -> std::fmt::Result {
        writeln!(output, "//! Generated Rust bindings for `{}::{}`.", self.abi.address, self.abi.name)?;
        writeln!(output, "//!")?;
        writeln!(output, "//! This file was auto-generated by aptos-rust-sdk-v2 codegen.")?;
    ...
    fn write_imports(&self, output: &mut String) -> std::fmt::Result {
        ...
        writeln!(output, "    transaction::{{EntryFunction, TransactionPayload}},")?;
    ...
    fn write_address_constant(&self, output: &mut String) -> std::fmt::Result {
        ...
        writeln!(output, "pub const MODULE_NAME: &str = \"{}\";", self.abi.name)?;
        writeln!(output)
    }
    ...
    fn write_struct(&self, output: &mut String, struct_def: &MoveStructDef) -> std::fmt::Result {
        ...
        // Documentation
        writeln!(output, "/// Move struct: `{}::{}`", self.abi.name, struct_def.name)?;
        ...
        // Derive macros
        writeln!(output, "#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]")?;
        ...
        if !struct_def.generic_type_params.is_empty() {
            ...
            writeln!(output, "pub struct {}<{}> {{", rust_name, type_params.join(", "))?;
        } else {
    ...
    fn write_entry_function(&self, output: &mut String, func: &MoveFunction) -> std::fmt::Result {
        ...
        if let Some(doc) = &enriched.doc {
            ...
            writeln!(output, "/// Entry function: `{}::{}`", self.abi.name, func.name)?;
            writeln!(output, "///")?;
        }
        ...
        if !enriched.type_param_names.is_empty() {
            writeln!(output, "/// * `type_args` - Type arguments: {}", enriched.type_param_names.join(", "))?;
        }
        ...
        writeln!(output, "    Ok(TransactionPayload::EntryFunction(entry_fn))")?;
        writeln!(output, "}}")?;
        writeln!(output)
    }
    ...
    fn write_view_function(&self, output: &mut String, func: &MoveFunction) -> std::fmt::Result {
        ...
        if let Some(doc) = &enriched.doc {
            ...
            writeln!(output, "/// View function: `{}::{}`", self.abi.name, func.name)?;
            writeln!(output, "///")?;
        }
        ...
        // Function signature
        let async_kw = if self.config.async_functions { "async " } else { "" };
        ...
        // Call view function
        let await_kw = if self.config.async_functions { ".await" } else { "" };
        writeln!(
            output,
